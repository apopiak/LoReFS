 ...
 * interposing vnodes.
 *
 * In lorefs, this mapping is implemented by a hashtable. Each bucket
...
makelfsnode(struct vfs *vfsp, struct loinfo *li)
{
	...
	if ((lfs = lfsfind(vfsp, li)) == NULL) {
		 ...
		 * implementation detail of lorefs, it should behave as a regular
...
lfsfind(struct vfs *vfsp, struct loinfo *li)
{
	 ...
	 * structure.  If the new mount lies in the lorefs hierarchy, then
	 * this will confuse lorefs, because the original vfsp (of the
 ...
 *
 * NB: A lorefs shadow vnode causes exactly one VN_HOLD() on the
 * underlying vnode.
 */
	...
	int a1, a2, a3, a4, a5, a6, a7, a8, a9;
{
	...
	if (lorefsdebug == level || (lorefsdebug > 10 && (lorefsdebug - 10) >= level))
