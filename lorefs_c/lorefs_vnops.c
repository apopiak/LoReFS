...
/*
 * Given a vnode of lorefs type, lookup nm name and
 * return a shadow vnode (of lorefs type) of the
 * real vnode found.
 *
 * Due to the nature of lorefs, there is a potential
 * looping in path traversal.
 *
 * starting from the mount point of an lorefs;
 * a loop is defined to be a traversal path
 * where the mount point or the real vnode of
 * the root of this lorefs is encountered twice.
	...
	pathname_t *realpnp)
{
	...
	*vpp = NULL;	/* default(error) case */
	...
	if (doingdotdot) {
		if ((vtol(dvp))->lo_looping & LO_LOOPING) {
			...
			if ((tvp == li->li_rootvp) && (vp == realvp(tvp))) {
				 ...
				 * return the rootvp
				 * Ex: /mnt/mnt/..
				 * where / has been lorefs-mounted
				 * onto /mnt.  Return the lorefs
				 * node mounted at /mnt.
				 */
				*vpp = tvp;
				VN_RELE(vp);
				 ...
				 * Ex: /net/X/net/X/Y
				 *
				 * If LO_AUTOLOOP (autofs/lorefs looping detected)
				 ...
				 * by the mounted lorefs, ie /net/X/net/X
				 */
				VN_RELE(tvp);
	...
	/*
	 * if the found vnode (vp) is not of type lorefs
	 * then we're just going to make a shadow of that
	 * vp and get out.
	 *
	 * If the found vnode (vp) is of lorefs type, and
	...
	if (!doingdotdot && vfs_matchops(vp->v_vfsp, lo_vfsops)) {
		 ...
		 * vp equals the root vp of the lorefs, directly
		...
		if (!((vtol(dvp))->lo_looping & LO_LOOPING)) {
			if (vp == li->li_rootvp) {
				 ...
				 * levels of lorefs indirections. Suppose vnode X
				 * in the current lookup has as its real vnode
				 * another lorefs node. Y = realvp(X) Y should be
				 * a lorefs node for the check to continue or Y
			 ...
			 *
			 * Now that an autofs/lorefs looping condition has been
			 ...
			 * to indicate the termination of the autofs/lorefs
			...
			if (vp->v_vfsp->vfs_vnodecovered == NULL) {
				 ...
				 * find the vnode covered by the lorefs rootvp.
				 */
				vp = li->li_rootvp;
				vp = vp->v_vfsp->vfs_vnodecovered;
				VN_RELE(tvp);
	...
	int flags)
{
	...
	/*
	 * The source and destination vnodes may be in different lorefs
	 ...
	 * /foo		- lorefs mount of /data/foo
	 * /bar		- read-only lorefs mount of /data/bar
	 *
	...
	int flags)
{
	 ...
	 * and that filesystem is read/write, but the lorefs filesystem is
	 ...
	 *		mount -F lorefs -o ro /foo /baz - all still in root
	 *			directory
	 ...
	 * filesystem mounted on top of lorefs, which only we know about.
	 */
...
rename:
	 ...
	 * the stack of lorefs filesystems, we need to recurse down the stack,
	 * checking to see if there are any instances of a filesystem mounted on
	 * top of lorefs. In order to keep on using the lorefs version of
	 * VOP_RENAME(), we make sure that while the target directory is of type
	 * lorefs, the source directory (the one used for getting the fs-specific
	 * version of VOP_RENAME()) is also of type lorefs.
	 */
...
#endif
	/* if cdir is lorefs vnode ptr get its real vnode ptr */
