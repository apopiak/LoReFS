...
static mntopts_t lorefs_mntopts;

static int lorefsinit(int, char *);
...
static struct modlfs modlfs = {
	&mod_fsops, "filesystem for lorefs", &vfw
};
	...
	struct cred *cr)
{
	 ...
	 *
	 * If a lorefs mount creates a mount loop (such that a lorefs vfs is
	 * mounted on an autofs node and that lorefs vfs points back to the
	 ...
	 * return EINTR. In such a case we don't want the lorefs vfs to be
	...
	/*
	 * Keep track of the number of mounted file systems
	 * so we can safely unmount it
	 */
...
lo_unmount(struct vfs *vfsp, int flag, struct cred *cr)
{
	...
	// reduce so mount count so we can safely unload the module
	lorefs_dec_mount_count();
	return (0);
}

/*
 * Find root of lorefs mount.
 */
static int
...
lo_statvfs(register struct vfs *vfsp, struct statvfs64 *sbp)
{
	 ...
	 * cached) is necessary to make lorefs work woth forced UFS unmounts.
...
lorefsinit(int fstyp, char *name)
{
	...
	if (error != 0) {
		cmn_err(CE_WARN, "lorefsinit: bad vfs ops template");
		return (error);
	}
	...
	if (error != 0) {
		...
		cmn_err(CE_WARN, "lorefsinit: bad vnode ops template");
		return (error);
	}
